...
class LfuCache extends BaseCache {
  ...
  async set({key, val, ttl}) {
    ...
    try {
      ...
      await this._channel.transaction((transaction) => {
        ...
        transaction.incrementSetMember(this._sortingKey, key);

        if (ttl) {
          const expirationScore = Math.ceil(new Date().getTime() / 1000) + ttl;
  ...
  async get(key) {
    ...
    try {
      ...
      this._channel.incrementSetMember(this._sortingKey, key);
